...
- 文生资产风格一致性：默认开启 `--asset-style-consistency`，会先写出 `text_scene_asset_style_spec.json` 和 `text_scene_image2_generation_plan.json`。image2 应先生成 anchor 物体，再把通过 QA 的 anchor 图作为后续物体的风格参考，避免材质、木色、金属和布料风格散掉。
...
- 资产从 0 生成：通过 `--asset-source-image-dir` 接入 image2 物体图目录，先做 source image QA，再使用 Trellis2 `512 + preprocess_image + 2048 texture + 500000 decimation` 默认高质量参数生成 GLB。
...
- image2 单物体图生成目前由 Codex 对话侧或外部工具完成；WebUI 后台任务只消费生成好的 `source_images/` 文件。生成时优先遵循 `text_scene_image2_generation_plan.json` 的 batch 和 per-object prompt。
