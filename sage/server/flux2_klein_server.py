#!/usr/bin/env python3
"""Small async Flux.2-klein HTTP server compatible with SAGE's FluxClient."""
...
class Flux2KleinRuntime:
    ...
    def _pipeline(self):
        ...
        with self._lock:
            ...
            print(f"[flux2-klein] loading pipeline from {self.args.model_path}", flush=True)
            ...
            self._pipe = pipe
            self._torch = torch
            print(f"[flux2-klein] ready on {self.args.device}", flush=True)
        return self._pipe, self._torch
...
def build_handler(jobs: JobStore, runtime: Flux2KleinRuntime):
    class Handler(BaseHTTPRequestHandler):
        server_version = "SAGEFlux2Klein/0.1"
            ...
            try:
                ...
                def worker() -> None:
                    try:
                        ...
                        print(f"[flux2-klein] job {job_id} failed:\n{trace}", flush=True)
        ...
        def log_message(self, fmt: str, *args: Any) -> None:
            print(f"[flux2-klein] {self.address_string()} {fmt % args}", flush=True)

    return Handler


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Serve local Flux.2-klein behind SAGE's async image API.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8084)
    parser.add_argument("--model-path", default="/data/xy/models/FLUX.2-klein-9B")
    parser.add_argument("--output-dir", default="/data/xy/SAGE_repro/flux2_klein_images")
...
def main() -> None:
    ...
    jobs = JobStore()
    runtime = Flux2KleinRuntime(args)
    httpd = ThreadingHTTPServer((args.host, args.port), build_handler(jobs, runtime))
    print(f"[flux2-klein] serving on http://{args.host}:{args.port}", flush=True)
    httpd.serve_forever()
